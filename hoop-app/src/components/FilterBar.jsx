...
const FilterBar = ({
  ...
  <div style={{
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
      ...
      {/* Month picker - only shows valid "Mmm YY" sheets */}
      <div style={{ position: 'relative' }} ref={monthMenuRef}>
        ...
        {showMonthMenu && (
          <div style={{
            ...
            gap: '4px', zIndex: 200, minWidth: '200px',
            boxShadow: '0 16px 48px rgba(0,0,0,0.7)',
          }}>
            {availMonths && availMonths.length > 0 ? availMonths.map(m => {
              const sel = selectedMonth === m;
              return (
                <button key={m} onClick={() => { setSelMonth(m); setFilterMode('month'); setShowMonthMenu(false); }} style={{
                  padding: '8px 4px', borderRadius: RADIUS.sm, border: sel ? 'none' : '1px solid rgba(91,155,213,0.1)',
