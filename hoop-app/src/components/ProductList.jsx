...
const ProductList = ({ colors, isDesktop }) => {
  ...
  const fetchProducts = useCallback(() => {
    setLoading(true);
    setError(false);
    axios.get(SCRIPT_URL, { params: { action: 'getProduk' }, timeout: 10000 })
      .then(response => {
  ...
  return (
    <div style={{ maxWidth: isDesktop ? '720px' : '100%', fontFamily: FONT }}>
      <SectionLabel colors={colors}>Product list</SectionLabel>
      ...
      <div style={{ position: 'relative', marginBottom: '18px' }}>
        ...
        <input
          type="text" placeholder="Search by SKU or name…"
      ...
      {loading && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[...Array(5)].map((_, i) => (
            <div key={i} style={{
              height: '58px', backgroundColor: colors.card,
              border: `1px solid ${colors.border}`, borderRadius: RADIUS.md, opacity: 1 - i * 0.15,
            }} />
          ))}
        </div>
      )}
      ...
      {error && !loading && (
        <div style={centeredBox(colors)}>
          <AlertIcon color={colors.error || '#EF4444'} />
          <span style={{ fontSize: TYPE.sm, color: colors.subText }}>Could not load products.</span>
      ...
      {!loading && !error && filtered.length === 0 && (
        <div style={centeredBox(colors)}>
          ...
          {search && (
            <button onClick={() => setSearch('')} style={{ ...retryBtn(), marginTop: '10px' }}>
              Clear search
            </button>
          )}
        </div>
...
const SectionLabel = ({ colors, children }) => (
  <div style={{ fontSize: TYPE.xs, fontWeight: '600', letterSpacing: '.08em', textTransform: 'uppercase', color: colors.subText, marginBottom: '12px' }}>
    {children}
  </div>
);
...
const centeredBox = (colors) => ({
  backgroundColor: colors.card, border: `1px solid ${colors.border}`,
  borderRadius: RADIUS.lg, padding: '48px 24px',
